...
func init(state: String, error_code: int = 0) -> void:
    ...
    if state == "Save":
        ...
        match error_code:
            ...
            _:
                push_error("Error: Alert_Window failed to find an error code: ", error_code)
                print("Error: Alert_Window failed to find an error code: ", error_code)
                window.queue_free()
