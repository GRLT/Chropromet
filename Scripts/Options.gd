...
@onready var window_mode_option := $CanvasLayer/BoxContainer/HWindow_Container/Window_Mode_Option

@onready var back := $CanvasLayer/BoxContainer/Back_Button

@onready var volume_slider := $CanvasLayer/BoxContainer/HVolume_Container/Volume_Slider
@onready var volume_label_value := $CanvasLayer/BoxContainer/HVolume_Container/Volume_Label_Value

@onready var max_fps_label := $CanvasLayer/BoxContainer/HFPS_Container/Max_FPS_Label
@onready var max_fps_option := $CanvasLayer/BoxContainer/HFPS_Container/Max_FPS_Option

@onready var vsync_check := $CanvasLayer/BoxContainer/HVsync_Container/Vsync_Check
...
func _ready():
	...
	volume_slider.drag_ended.connect(volume_slider_ended)
...
func vsync_check_toggled(state:bool):
	if state:
		DisplayServer.window_set_vsync_mode(DisplayServer.VSYNC_ENABLED)
	else:
		DisplayServer.window_set_vsync_mode(DisplayServer.VSYNC_DISABLED)

func volume_slider_ended(_value_changed):
	var current_volume = str(int(volume_slider.value))
...
func max_fps_item_change(id):
	Engine.set_max_fps(int(max_fps_option.get_item_text(id)))
	
func window_mode_item_change(id):
	match id:
		0:
